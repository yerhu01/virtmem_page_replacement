/*
 * This does everything as in virtmem.c as well as Random and Not Frequently Used replacement algorithms
 * use --replace=random and --replace=nfu
 */
...
#define REPLACE_RANDOM 5
#define REPLACE_NFU 6
...
int r= 0;   //used to generate random number
int nfu_time = 0; //keeps track of time
/*
...
struct page_table_entry {
    long page_num;
    int dirty; //for clock
	int modified;  //for lru (keeps track of time)
    int free;
	int nfu_count; //for nfu
};
...
long resolve_address(long logical, int memwrite)
{
	...
	//reset count at end of clock cycle (FOR NFU)
	if(page_replacement_scheme == REPLACE_NFU && nfu_time == CLOCK_CYCLE){
		nfu_time = 0;
		for(i=0;i<size_of_memory;i++){
			page_table[i].nfu_count = 0;
		}
	}
    ...
    if (frame != -1) {     
		...
		if(page_replacement_scheme == REPLACE_NFU){
			//page was referenced
			page_table[frame].nfu_count++;
			nfu_time++;
			swap_ins++; //increased because page was modified
		}
        return effective;
    ...
    if (frame != -1) {
		...
		if(page_replacement_scheme == REPLACE_NFU){
			page_table[frame].nfu_count = 0; //initialize to 0
			nfu_time++;
		}
		...
		if(page_replacement_scheme == REPLACE_FIFO){
			...
			effective = (frame << size_of_frame) | offset;
			return effective;
		}else if(page_replacement_scheme == REPLACE_RANDOM){
			//get random frame
			if(r%2==0){
				srand(r+7*time(NULL));
			}else{
				srand(9*r-2*time(NULL));
			}
			r = rand()%size_of_memory;
			frame = (long)r;
			//printf("%d \n", r);
			//replace page at random frame with new page
			page_table[frame].page_num = page;
			page_table[frame].free = FALSE;
			swap_outs++;
			swap_ins++;
			effective = (frame << size_of_frame) | offset;
			return effective;
		}else if(page_replacement_scheme == REPLACE_NFU){
			//find frame with lowest count
			frame = 0;
			int lowest_count = page_table[0].nfu_count;
			for(i = 0; i < size_of_memory; i++){
				if (lowest_count > page_table[i].nfu_count){
					lowest_count = page_table[i].nfu_count;
					frame = i;
				}
			}
			
			//replace page with lowest count with new page
			page_table[frame].page_num = page;
			page_table[frame].free = FALSE;
			page_table[frame].nfu_count = 0; //update nfu_count
			nfu_time++;
			swap_outs++;
			swap_ins++;
			effective = (frame << size_of_frame) | offset;
    ...
    for (i=1; i < argc; i++) {
        if (strncmp(argv[i], "--replace=", 9) == 0) {
            ...
            if (strcmp(s, "fifo") == 0) {
				...
				page_replacement_scheme = REPLACE_RANDOM;
			} else if (strcmp(s, "nfu") == 0){
				page_replacement_scheme = REPLACE_NFU;
			} else{
